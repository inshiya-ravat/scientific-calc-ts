...
const operationElement:HTMLElement = document.querySelector('.keys')!
...
function handleTrignometryAdvanceMathFunction(e:Event){
    if(e.target instanceof HTMLSelectElement){
        ...
        if(trignometryOperation instanceof HTMLOptionElement && advMathOperation instanceof HTMLOptionElement){
        ...
        if(((IS_DIGIT) || (IS_ARITHEMATIC_OPERATION)) && name !== undefined && name != "calc") 
        {
            ...
            switch (name){
                ...
                case CALCULATOR_OPERATION.square:{
                    const val = is2ndEnabled ? Math.pow(Number(getExpression()[getExpression().length-1]), 3) : Math.pow(Number(getExpression()[getExpression().length-1]), 2)
